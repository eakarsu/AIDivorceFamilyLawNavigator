/**
 * Custom Feature: state-guidance
 * State-aware legal guidance
 *
 * POST /api/ai/state-guidance
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_STATE_GUIDANCE_KEY
...
const FEATURE_KEY = process.env.FEATURE_STATE_GUIDANCE_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: State-aware legal guidance.
...
async function callLLM(userPayload) {
  ...
  const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIDivorceFamilyLawNavigator - state-guidance',
    },
    body: JSON.stringify({
...
router.post('/state-guidance', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_STATE_GUIDANCE_KEY');
    const ai = await callLLM({ feature: 'state-guidance', goal: 'State-aware legal guidance', input: payload });
    return res.json({
      ok: true,
      feature: 'state-guidance',
      endpoint: '/api/ai/state-guidance',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[state-guidance] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/state-guidance/health', (req, res) => {
  res.json({
    feature: 'state-guidance',
    endpoint: '/api/ai/state-guidance',
